$(() => {
    ...
    //CREATE OBJECT OF SENTIMENT WORDS AND THEIR FREQUENCY - WIP 
    //CHECK FOR REPEATS OF A WORD IN AN ARRAY - WIP 
    function calculateSentimentWordFrequency(array) {
        ...
        for (wordIndex = 0; wordIndex < sortedArray.length; wordIndex++) {
            ...
            if (sortedArray[wordIndex] === sortedArray[subsequentIndex]) {
                frequency++;
                console.log(sortedArray[wordIndex] + ': ' + frequency);
            } else {
                subsequentIndex++;
            }
        }
    }
let testArray = ['hello', 'hello', 'hi', 'boo', 'hey', 'boo'];
console.log(calculateSentimentWordFrequency(testArray));
